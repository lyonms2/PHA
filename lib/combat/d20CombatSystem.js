...
import { calcularMultiplicadorElemental as calcMultElemental } from '@/lib/combat/core/elementalSystem';
import { ELEMENTAL_BALANCE } from '@/app/avatares/sistemas/balance/elementalBalance.js';
...
/**
 * Verifica vantagem elemental usando sistema unificado (9 elementos)
 * @param {string} elementoAtacante
 * @param {string} elementoDefensor
 * @returns {string} 'oposto', 'vantagem', 'vantagem_especial', 'desvantagem', 'resistente', 'neutro'
 */
export function verificarVantagemElemental(elementoAtacante, elementoDefensor) {
  ...
  // Usa o sistema unificado que suporta 9 elementos (incluindo Void e Aether)
  const resultado = calcMultElemental(elementoAtacante, elementoDefensor);
  return resultado.tipo;
}

/**
 * Calcula multiplicador de dano elemental
 * Usa valores centralizados de elementalBalance.js
 * @param {string} tipoVantagem - Tipo de vantagem elemental
...
export function calcularMultiplicadorElemental(tipoVantagem) {
  switch (tipoVantagem) {
    ...
    case 'vantagem_especial':
      return ELEMENTAL_BALANCE.VANTAGEM_MULTIPLICADOR; // 1.15
    case 'resistente':
    case 'desvantagem':
      return ELEMENTAL_BALANCE.DESVANTAGEM_MULTIPLICADOR; // 0.9
    default:
      return ELEMENTAL_BALANCE.NEUTRO_MULTIPLICADOR; // 1.0
  }
}
