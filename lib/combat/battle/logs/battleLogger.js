...
export function formatAttackLog(log, detalhes) {
  ...
  // Adicionar detalhes de cálculo se disponíveis
  if (detalhes) {
    texto += `\n📊 Cálculo:`;
    if (detalhes.danoBase) texto += ` Base=${detalhes.danoBase}`;
    if (detalhes.focoBonus) texto += ` +Foco(${detalhes.focoBonus})`;
    if (detalhes.vinculoBonus) texto += ` +Vínculo(${detalhes.vinculoBonus})`;
    if (detalhes.elementalMult && detalhes.elementalMult !== 1) texto += ` ×${detalhes.elementalMult}(Elem)`;
    if (critico) texto += ` ×1.5(Crít)`;
    if (bloqueado) texto += ` ×0.5(Def)`;
    if (detalhes.reducaoDefesa) texto += ` -${detalhes.reducaoDefesa}(RES)`;
  }
