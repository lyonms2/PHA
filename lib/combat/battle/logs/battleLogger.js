...
export function formatAttackLog(log, detalhes) {
  const { jogador, alvo, errou, invisivel, esquivou, chanceAcerto, dano, critico, elemental, bloqueado, rouboVida } = log;
  ...
  if (detalhes) {
    ...
    if (detalhes.bonusVinculo) texto += `\n  • Vínculo: ${detalhes.bonusVinculo}`; // CORRIGIDO: era vinculoBonus
    if (detalhes.penalidadeExaustao) texto += `\n  • Exaustão: ${detalhes.penalidadeExaustao}`;
    if (detalhes.sinergia) texto += `\n  ✨ Sinergia: ${detalhes.sinergia}`;
  ...
  // Adicionar roubo de vida se houver
  if (rouboVida && rouboVida > 0) {
    texto += `\n💚 Roubo de Vida: +${rouboVida} HP`;
  }
...
export function formatAbilityLog(log, detalhes) {
  const { jogador, alvo, habilidade, errou, invisivel, esquivou, dano, cura, critico, elemental, bloqueado, numGolpes, efeitos, rouboVida } = log;
  ...
  if (dano > 0) {
    ...
    if (detalhes) {
      ...
      if (detalhes.sinergia) texto += `  ✨ Sinergia: ${detalhes.sinergia}\n`;
