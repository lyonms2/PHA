...
export function processAttack(battleState, attacker, defender) {
  ...
  // Verificar dreno de energia do escudo energético (AETHER)
  const escudoEnergetico = (defender.effects || []).find(ef => ef.tipo === 'escudo_energetico');
  let energiaDrenada = 0;
  let newAttackerEnergia = attacker.energia;

  if (escudoEnergetico && dano > 0 && escudoEnergetico.drenaEnergiaAtacante) {
    energiaDrenada = escudoEnergetico.drenaEnergiaAtacante;
    newAttackerEnergia = Math.max(0, attacker.energia - energiaDrenada);
    console.log('✨⚡ [DRENO ENERGIA] Escudo Energético ativado!', {
      danoRecebido: dano,
      energiaDrenada,
...
export function processAbility(battleState, attacker, defender, habilidade) {
  ...
  // Verificar dreno de energia do escudo energético (AETHER) - só para habilidades ofensivas
  const escudoEnergetico = (tipoNormalizado === 'ofensivo' || tipoNormalizado === 'ofensiva') ?
    (defender.effects || []).find(ef => ef.tipo === 'escudo_energetico') : null;
  let energiaDrenada = 0;

  if (escudoEnergetico && dano > 0 && escudoEnergetico.drenaEnergiaAtacante) {
    energiaDrenada = escudoEnergetico.drenaEnergiaAtacante;
    console.log('✨⚡ [DRENO ENERGIA HABILIDADE] Escudo Energético ativado!', {
      danoRecebido: dano,
      energiaDrenada,
