...
export function processAttack(battleState, attacker, defender) {
  ...
  // Verificar contra-ataques de efeitos defensivos
  const escudoFlamejante = (defender.effects || []).find(ef => ef.tipo === 'escudo_flamejante');
  const aegisSagrado = (defender.effects || []).find(ef => ef.tipo === 'aegis_sagrado');
  ...
  if (aegisSagrado && dano > 0) {
    // Reflexo: 15% do dano recebido volta como dano de luz
    const danoReflexo = Math.floor(dano * 0.15);
    danoContraAtaque += danoReflexo;
    console.log('✨ [REFLEXO] Aegis Sagrado ativado!', {
      danoRecebido: dano,
      danoReflexo,
      defensor: defender.nome,
      atacante: attacker.nome
  ...
  // Verificar dreno de energia de efeitos defensivos (VOID ou AETHER)
  const efeitoDreno = (defender.effects || []).find(ef =>
    (ef.tipo === 'escudo_energetico' || ef.tipo === 'reducao_dano') && ef.drenaEnergiaAtacante
  );
  let energiaDrenada = 0;
  let newAttackerEnergia = newEnergy; // Usar newEnergy que já tem o custo do ataque (-10)

  if (efeitoDreno && dano > 0 && efeitoDreno.drenaEnergiaAtacante) {
    energiaDrenada = efeitoDreno.drenaEnergiaAtacante;
    newAttackerEnergia = Math.max(0, newEnergy - energiaDrenada); // Drenar do newEnergy
    const nomeEfeito = efeitoDreno.tipo === 'reducao_dano' ? 'Campo de Anulação' : 'Escudo Energético';
    console.log(`✨⚡ [DRENO ENERGIA] ${nomeEfeito} ativado!`, {
      danoRecebido: dano,
      energiaDrenada,
      energiaAntes: newEnergy,
      energiaDepois: newAttackerEnergia,
      defensor: defender.nome,
      atacante: attacker.nome
    });
  }
...
export function processAbility(battleState, attacker, defender, habilidade) {
  ...
  if (efeitosHabilidade && Array.isArray(efeitosHabilidade)) {
    ...
    for (const efeito of efeitosHabilidade) {
      ...
      // ===== PROCESSAR ANULAR BUFFS (VOID) =====
      if (nomeEfeito === 'anula_buffs') {
        const alvoAnulacao = (typeof efeito === 'object' ? efeito.alvo : null) || habilidade.alvo || 'oponente';
        if (alvoAnulacao === 'self' || alvoAnulacao === 'proprio') {
          // Remover todos os buffs do atacante (manter apenas debuffs)
          const efeitosValidos = newAttackerEffects.filter(ef => ef && ef.tipo);
          const buffsRemovidos = efeitosValidos.filter(ef => ehBuff(ef.tipo));
          newAttackerEffects = efeitosValidos.filter(ef => !ehBuff(ef.tipo));
          console.log(`🕳️💜 [ANULAR BUFFS] Removidos ${buffsRemovidos.length} buffs de ${attacker.nome}:`, buffsRemovidos.map(b => b.tipo).join(', '));
        } else {
          // Remover todos os buffs do defensor (manter apenas debuffs) - VOID SPECIAL
          const efeitosValidos = newDefenderEffects.filter(ef => ef && ef.tipo);
          const buffsRemovidos = efeitosValidos.filter(ef => ehBuff(ef.tipo));
          newDefenderEffects = efeitosValidos.filter(ef => !ehBuff(ef.tipo));
          console.log(`🕳️💜 [ANULAR BUFFS] Ruptura Dimensional removeu ${buffsRemovidos.length} buffs de ${defender.nome}:`, buffsRemovidos.map(b => b.tipo).join(', '));
        }
        efeitosAplicados.push(nomeEfeito);
        continue; // Não adicionar como efeito de status
      }
      ...
      // Duração direta - não precisa de ajuste para efeitos "self"
      // Timing: Turno 1 aplica (N) → Turno 2 início decrementa (N-1) → ... → Turno N+1 início expira (0)
      // Resultado: Efeito ativo por N turnos completos
      const turnosRestantesInicial = duracaoEfeito;
      ...
      const efeitoObj = {
        ...
        ...(efeitoDicionario.reducao_forca && { reducaoForca: efeitoDicionario.reducao_forca }),
        ...(efeitoDicionario.reducao_foco && { reducaoFoco: efeitoDicionario.reducao_foco }),
        ...
        ...(efeitoDicionario.reducao_dano_recebido && { reducaoDanoRecebido: efeitoDicionario.reducao_dano_recebido }),
        ...(efeitoDicionario.acerto_garantido && { acertoGarantido: efeitoDicionario.acerto_garantido }),
        ...(efeitoDicionario.drena_energia_atacante && { drenaEnergiaAtacante: efeitoDicionario.drena_energia_atacante }),
      };
  ...
  // Verificar contra-ataques de efeitos defensivos (só para habilidades ofensivas)
  ...
  const aegisSagrado = (tipoNormalizado === 'ofensivo' || tipoNormalizado === 'ofensiva') ?
    (defender.effects || []).find(ef => ef.tipo === 'aegis_sagrado') : null;
  let danoContraAtaque = 0;
  ...
  // Verificar dreno de energia de efeitos defensivos (VOID ou AETHER) - só para habilidades ofensivas
  const efeitoDreno = (tipoNormalizado === 'ofensivo' || tipoNormalizado === 'ofensiva') ?
    (defender.effects || []).find(ef =>
      (ef.tipo === 'escudo_energetico' || ef.tipo === 'reducao_dano') && ef.drenaEnergiaAtacante
    ) : null;
  let energiaDrenada = 0;
  ...
  if (efeitoDreno && dano > 0 && efeitoDreno.drenaEnergiaAtacante) {
    energiaDrenada = efeitoDreno.drenaEnergiaAtacante;
    const nomeEfeito = efeitoDreno.tipo === 'reducao_dano' ? 'Campo de Anulação' : 'Escudo Energético';
    console.log(`✨⚡ [DRENO ENERGIA HABILIDADE] ${nomeEfeito} ativado!`, {
      danoRecebido: dano,
      energiaDrenada,
      energiaAntes: attacker.energy,
      energiaDepois: Math.max(0, attacker.energy - energiaDrenada),
      defensor: defender.nome,
      atacante: attacker.nome
    });
  }
  ...
  return {
    ...
    attacker: {
      ...attacker,
      energy: Math.max(0, newEnergy - energiaDrenada),
      hp: newAttackerHp,
      effects: newAttackerEffects
    },
    defender: {
