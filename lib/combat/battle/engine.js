...
export function processAbility(battleState, attacker, defender, habilidade) {
  ...
  if (tipoNormalizado === 'ofensivo' || tipoNormalizado === 'ofensiva') {
    ...
    // Calcular dano (com modificadores do defensor)
  ...
  if (efeitosHabilidade && Array.isArray(efeitosHabilidade)) {
    ...
    for (const efeito of efeitosHabilidade) {
      ...
      const efeitosApenasCalculo = ['auto_cura', 'cura_instantanea'];
      ...
      if (nomeEfeito === 'limpar_debuffs') {
        ...
        if (alvoLimpeza === 'self' || alvoLimpeza === 'proprio') {
          // Remover todos os debuffs do atacante
          const debuffsRemovidos = newAttackerEffects.filter(ef => !ehBuff(ef.tipo));
          newAttackerEffects = newAttackerEffects.filter(ef => ehBuff(ef.tipo));
          console.log(`✨ [LIMPAR DEBUFFS] Removidos ${debuffsRemovidos.length} debuffs de ${attacker.nome}:`, debuffsRemovidos.map(d => d.tipo).join(', '));
        } else {
          // Remover todos os debuffs do defensor
          const debuffsRemovidos = newDefenderEffects.filter(ef => !ehBuff(ef.tipo));
          newDefenderEffects = newDefenderEffects.filter(ef => ehBuff(ef.tipo));
...
export function processEffects(combatant) {
  ...
  const stunEffect = combatant.effects.find(ef => stunEffects.includes(ef.tipo));

  if (stunEffect) {
    console.log(`😵 [PROCESSEFFECTS] ${combatant.nome} está ${stunEffect.tipo} e não pode agir!`);

    // Decrementar e remover efeitos de atordoamento
    const newEffects = combatant.effects.map(ef => {
      if (stunEffects.includes(ef.tipo)) {
        return {
          ...ef,
      ...
      if (stunEffects.includes(ef.tipo) && ef.turnosRestantes <= 0) {
