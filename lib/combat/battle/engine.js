...
export function processAttack(battleState, attacker, defender) {
  ...
  return {
    ...
    log: {
      ...
      contraAtaque: temContraAtaque,
      elemental: elemental.tipo,
      detalhes: `${attacker.nome} atacou ${defender.nome}! Dano: ${dano}${critico ? ' (CRÍTICO!)' : ''}${elemental.tipo !== 'normal' ? ` [${elemental.tipo}]` : ''}${defender.defending ? ' (BLOQUEADO -50%)' : ''}`
    }
  };
}
...
export function processDefend(battleState, defender) {
  ...
  return {
    ...
    log: {
      ...
      jogador: defender.nome,
      energiaRecuperada,
      detalhes: `${defender.nome} defendeu! Energia +${energiaRecuperada} (Próximo ataque recebido -50%)`
    }
  };
}
...
export function processAbility(battleState, attacker, defender, habilidade) {
  ...
  return {
    ...
    log: {
      ...
      numGolpes,
      efeitos: efeitosAplicados,
      detalhes: `${attacker.nome} usou ${habilidade.nome}! ${dano > 0 ? `Dano: ${dano}${numGolpes > 1 ? ` (${numGolpes}× golpes)` : ''}${critico ? ' (CRÍTICO!)' : ''}${elemental.tipo !== 'normal' ? ` [${elemental.tipo}]` : ''}` : ''}${cura > 0 ? `Cura: ${cura}` : ''}${efeitosAplicados.length > 0 ? ` | Efeitos: ${efeitosAplicados.join(', ')}` : ''}`
    }
  };
}
