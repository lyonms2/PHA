...
export function calcularHPMaximo(avatarOuResistencia, nivel, raridade) {
  ...
  // Cálculo base: HP = resistencia × 20 (2x do original)
  const hpBase = resistencia * 20;

  // Bônus de nível: +10 HP por nível (2x do original)
  const bonusNivel = nivelFinal * 10;

  // Bônus de raridade (opcional) - 2x do original
  let bonusRaridade = 0;
  if (raridadeFinal) {
    switch (raridadeFinal) {
      case 'Lendário':
      case 'Legendário': // Suporte para ambas grafias
        bonusRaridade = 200; // 2x (antes: 100)
        break;
      case 'Raro':
        bonusRaridade = 100; // 2x (antes: 50)
        break;
      case 'Comum':
      default:
        bonusRaridade = 0;
 ...
 *
 * @example
 * const hpGanho = calcularGanhoHPPorNivel(3); // 30 HP (3 níveis × 10 HP)
 */
export function calcularGanhoHPPorNivel(niveisGanhos) {
  return niveisGanhos * 10; // 2x (antes: 5)
}
 ...
 * BALANCEAMENTO 2.0: Valores dobrados para batalhas mais estratégicas
 */
export const BONUS_HP_POR_RARIDADE = {
  Comum: 0,
  Raro: 100,      // 2x (antes: 50)
  'Lendário': 200, // 2x (antes: 100)
  'Legendário': 200 // Suporte para ambas grafias
};

/**
 * Fórmula base de HP
 * BALANCEAMENTO 2.0: Multiplicadores dobrados (2x HP)
 */
export const FORMULA_HP = {
  HP_POR_RESISTENCIA: 20, // 2x (antes: 10)
  HP_POR_NIVEL: 10,       // 2x (antes: 5)
  descricao: 'HP = (resistencia × 20) + (nivel × 10) + bônus_raridade'
};
