...
/**
 * Calcula o multiplicador elemental
 */
export function calcularMultiplicadorElemental(elementoAtacante, elementoDefensor) {
  ...
  if (vantagens[elementoAtacante] === elementoDefensor) {
    return { mult: 1.5, tipo: 'vantagem' };
  }
  if (vantagens[elementoDefensor] === elementoAtacante) {
 ...
 */
export function calcularDanoAtaque(atacante, defensor, opcoesDefensor = {}) {
  const forca = atacante.forca ?? 10;
  const foco = atacante.foco ?? 10;
  const agilidade = atacante.agilidade ?? 10;
  const vinculo = atacante.vinculo ?? 0;
  const exaustao = atacante.exaustao ?? 0;
  const elemento = atacante.elemento || 'Neutro';

  const resistenciaOponente = defensor.resistencia ?? 10;
  const agilidadeOponente = defensor.agilidade ?? 10;
  ...
  // ===== REDUÇÃO POR DEFESA =====
  const temDefesaAumentada = efeitosOponente.some(ef => ef.tipo === 'defesa_aumentada');
 ...
 */
export function calcularDanoHabilidade(atacante, defensor, habilidade, opcoesDefensor = {}) {
  const forca = atacante.forca ?? 10;
  const foco = atacante.foco ?? 10;
  const agilidade = atacante.agilidade ?? 10;
  const vinculo = atacante.vinculo ?? 0;
  ...
  if (habilidade.tipo === 'Ofensiva' || habilidade.tipo === 'Controle' || (habilidade.dano_base && habilidade.dano_base > 0)) {
    ...
    const temDefesaAumentada = efeitosOponente.some(ef => ef.tipo === 'defesa_aumentada');
    const multiplicadorDefesa = temDefesaAumentada ? 2.0 : 1.0;
    const reducaoResistencia = (resistenciaOponente * 0.4) * multiplicadorDefesa;
