...
export function calcularDanoAtaque({
  ...
  if (defenderModifiers.resistencia && defenderModifiers.resistencia > 0) {
    ...
    console.log(`%c🛡️ SINERGIA DEFENSIVA (Defensor se protege): +${percentual}%`,
      ...
      {
        'Resistência Base Defensor': Math.floor(resistenciaAntesSinergia),
        'Com Sinergia Defensor': Math.floor(resistenciaFinal),
        'Diferença': `+${Math.floor(resistenciaFinal - resistenciaAntesSinergia)}`
      }
    );
  }
