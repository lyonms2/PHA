...
export function calcularDanoAtaque({
  ...
  // Verificar se defensor tem efeito de redução de dano (Campo de Anulação do VOID ou Escudo Energético do AETHER)
  const efeitoReducaoDano = opponentEffects.find(ef =>
    (ef.tipo === 'reducao_dano' || ef.tipo === 'escudo_energetico') && ef.reducaoDanoRecebido
  );
  if (efeitoReducaoDano) {
    ...
    const nomeEfeito = efeitoReducaoDano.tipo === 'escudo_energetico' ? 'Escudo Energético' : 'Campo de Anulação';
    console.log(`🛡️✨ [REDUÇÃO DANO - ${nomeEfeito}] ${danoAntesReducao} × ${(1 - percentualReducao).toFixed(2)} = ${dano} (-${Math.floor(percentualReducao * 100)}%)`);
  }
...
export function calcularDanoHabilidade({
  ...
  // DEBUG: Verificar se propriedade existe e valor
  console.log('🔍 [DEBUG PENETRAÇÃO]', {
    nomeHabilidade: habilidade.nome,
    ignoraDefesa: habilidade.ignora_defesa,
    penetracao,
    condicao: penetracao > 0,
    habilidadeCompleta: habilidade
  });
  ...
  if (efeitoReducaoDano) {
    ...
    dano = Math.floor(dano * (1 - percentualReducao));
    const nomeEfeito = efeitoReducaoDano.tipo === 'escudo_energetico' ? 'Escudo Energético' : 'Campo de Anulação';
    console.log(`🛡️✨ [REDUÇÃO DANO HABILIDADE - ${nomeEfeito}] ${danoAntesReducao} × ${(1 - percentualReducao).toFixed(2)} = ${dano} (-${Math.floor(percentualReducao * 100)}%)`);
  }
