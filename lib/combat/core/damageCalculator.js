...
export function calcularDanoAtaque({
  ...
  // Aplicar modificadores de sinergia do DEFENSOR (aumenta ou reduz resistência)
  let resistenciaFinal = resistenciaOponente;
  if (defenderModifiers.resistencia) {
    resistenciaFinal = resistenciaOponente * (1 + defenderModifiers.resistencia);
    const percentual = Math.floor(defenderModifiers.resistencia * 100);

    console.log(`%c🛡️ SINERGIA DE RESISTÊNCIA (Inimigo): ${percentual >= 0 ? '+' : ''}${percentual}%`,
      'color: #ff6b6b; font-weight: bold',
      {
        'Resistência Base': Math.floor(resistenciaOponente),
        'Com Sinergia': Math.floor(resistenciaFinal),
        'Diferença': `${percentual >= 0 ? '+' : ''}${Math.floor(resistenciaFinal - resistenciaOponente)}`
      }
    );
  }
...
export function calcularDanoHabilidade({
  ...
  if (defenderModifiers.resistencia) {
    ...
    console.log(`%c🛡️ SINERGIA DE RESISTÊNCIA (Inimigo): ${percentual >= 0 ? '+' : ''}${percentual}%`,
      'color: #ff6b6b; font-weight: bold',
      {
        'Resistência Base Inimigo': Math.floor(resistenciaOponente),
        'Resistência com Sinergia': Math.floor(resistenciaFinal),
        'Diferença': `${percentual >= 0 ? '+' : ''}${Math.floor(resistenciaFinal - resistenciaOponente)}`
      }
    );
  }
