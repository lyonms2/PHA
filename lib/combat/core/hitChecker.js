...
export function testarAcertoAtaque({ agilidade, agilidadeOponente, attackerEffects = [], opponentEffects = [], defenderModifiers = {}, attackerModifiers = {} }) {
  // Verificar se atacante tem acerto garantido (Campo de Anulação)
  const temAcertoGarantido = attackerEffects.some(ef => ef.tipo === 'reducao_dano' && ef.acertoGarantido);
  if (temAcertoGarantido) {
    console.log('🎯💜 [ACERTO GARANTIDO] Campo de Anulação ativo - 100% de acerto!');
    return {
      acertou: true,
      chanceAcerto: 100,
      esquivou: false,
...
export function testarAcertoHabilidade({ chanceAcertoBase = 100, agilidadeOponente, attackerEffects = [], opponentEffects = [], defenderModifiers = {}, attackerModifiers = {} }) {
  ...
  const temAcertoGarantido = attackerEffects.some(ef => ef.tipo === 'reducao_dano' && ef.acertoGarantido);
  if (temAcertoGarantido) {
    console.log('🎯💜 [ACERTO GARANTIDO HABILIDADE] Campo de Anulação ativo - 100% de acerto!');
    return {
      acertou: true,
      chanceAcerto: 100,
      esquivou: false,
