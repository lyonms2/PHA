 ...
 */
export function testarAcertoAtaque({ agilidade, agilidadeOponente, opponentEffects = [], defenderModifiers = {}, attackerModifiers = {} }) {
  ...
  // Aplicar modificadores de sinergia de evasão do defensor (ex: +15% = reduz chance de acerto em 15 pontos)
  if (defenderModifiers.evasao_mult) {
    const evasaoPercentual = (defenderModifiers.evasao_mult - 1.0) * 100;
 ...
 * @returns {Object} { acertou: boolean, chanceAcerto: number, esquivou: boolean, invisivel: boolean }
 */
export function testarAcertoHabilidade({ chanceAcertoBase = 100, agilidadeOponente, opponentEffects = [], defenderModifiers = {}, attackerModifiers = {} }) {
  // Verificar buffs de evasão do oponente
  const temInvisibilidade = opponentEffects.some(ef => ef.tipo === 'invisivel' || ef.tipo === 'invisível');
  const temEvasaoAumentada = opponentEffects.some(ef => ef.tipo === 'evasao_aumentada');
  const temVelocidadeAumentada = opponentEffects.some(ef => ef.tipo === 'velocidade' || ef.tipo === 'velocidade_aumentada');
  ...
  // Aplicar modificadores de sinergia de evasão do defensor
  if (defenderModifiers.evasao_mult) {
    const evasaoPercentual = (defenderModifiers.evasao_mult - 1.0) * 100;
    bonusEvasao += evasaoPercentual;
  }
