...
/**
 * Get documents by query (alternative syntax)
 * @param {string} collectionName - Name of the collection
 * @param {Array} whereConditions - Array of {field, operator, value} objects
 * @param {Object} options - Optional orderBy and limit
 * @returns {Promise<Array>} Array of documents
 */
export async function getDocumentsByQuery(collectionName, whereConditions = [], options = {}) {
  try {
    const colRef = collection(db, collectionName);
    let q = colRef;

    // Apply where clauses from array of objects
    ...
    console.error(`Error getting documents by query from ${collectionName}:`, error);
    throw error;
  }
}
