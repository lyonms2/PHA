...
export const HUNTER_RANKS = {
  F: {
    ...
    descontoMercado: 0,
    descontoMerge: 0,
    multiplicadorRecompensas: 1.0  // +0%
  },
  E: {
    nome: 'E',
    minXp: 1000,
    ...
    descontoMercado: 0,
    descontoMerge: 0,
    multiplicadorRecompensas: 1.05  // +5%
  },
  D: {
    nome: 'D',
    minXp: 4000,
    ...
    descontoMerge: 0,
    multiplicadorRecompensas: 1.10  // +10%
  },
  C: {
    nome: 'C',
    minXp: 12000,
    ...
    descontoMerge: 0.02,
    multiplicadorRecompensas: 1.15  // +15%
  },
  B: {
    nome: 'B',
    minXp: 32000,
    ...
    descontoMerge: 0.04,
    multiplicadorRecompensas: 1.20  // +20%
  },
  A: {
    nome: 'A',
    minXp: 82000,
    ...
    descontoMerge: 0.06,
    multiplicadorRecompensas: 1.25  // +25%
  },
  S: {
    nome: 'S',
    minXp: 202000,
    ...
    descontoMerge: 0.08,
    multiplicadorRecompensas: 1.30  // +30%
  },
  SS: {
    nome: 'SS',
    minXp: 502000,
    ...
    bonusInvocacaoLendario: 0.015,
    descontoMercado: 0.12,
    descontoMerge: 0.10,
    multiplicadorRecompensas: 1.40  // +40%
  }
};
...
export function getResumoBonusRank(rank) {
  ...
  if (rank.multiplicadorRecompensas > 1.0) {
    bonus.push(`+${Math.round((rank.multiplicadorRecompensas - 1.0) * 100)}% recompensas`);
  }
...
/**
 * Aplica multiplicador de recompensas do Hunter Rank
 * Usado principalmente para missões diárias e outras recompensas
 */
export function aplicarMultiplicadorRecompensas(recompensas, rank) {
  const mult = rank.multiplicadorRecompensas || 1.0;
...
/**
 * Retorna o limite de avatares baseado no Hunter Rank
 * Quanto maior o rank, mais avatares o jogador pode ter
 */
export function getLimiteAvatares(hunterRank) {
  const limites = {
    'F': 10,
    'E': 12,
    'D': 15,
  ...
  // Se receber objeto rank, pegar o nome
  const rankNome = typeof hunterRank === 'string' ? hunterRank : hunterRank.nome;

  return limites[rankNome] || 10; // Padrão: 10 avatares
}
