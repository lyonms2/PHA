...
function serializeBattle(battleData) {
  try {
    // Converter para JSON e depois voltar para objeto (remove funções e referências)
...
export async function setBattle(battleId, battleData) {
  ...
  try {
    ...
    console.error(`❌ [SESSION STORAGE] BattleId:`, battleId);
    console.error(`❌ [SESSION STORAGE] Stack:`, error.stack);
    throw error;
  }
}
