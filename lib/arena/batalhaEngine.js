...
export function calcularDano(atacante, habilidade, defensor, critico = false) {
  // Calcular dano baseado 100% em stats (sem dano base fixo)
  const statValue = atacante[habilidade.stat_primario] || atacante.forca;
  const multiplicador = habilidade.multiplicador_stat || 3.5;
  const danoBase = habilidade.dano_base || 0;
  let dano = danoBase + (statValue * multiplicador);

  // Bônus de nível (se tiver)
  if (atacante.nivel) {
  ...
  battleLogger.logVantagemElemental(atacante.elemento, defensor.elemento, vantagemElemental);
  dano *= vantagemElemental;

  // Crítico
  if (critico) {
    battleLogger.logCritico();
  ...
  battleLogger.logResistencia(defensor, Math.floor(reducao));
  dano -= reducao;
  ...
  if (bonusSinergia > 0) {
    battleLogger.logSinergia(atacante, defensor, bonusSinergia);
  }
  ...
  const danoFinal = Math.max(1, Math.floor(dano));

  // Log detalhado do dano
  battleLogger.logDano(danoFinal, defensor, {
    danoBase,
    multiplicadorStat: multiplicador,
    statValue,
    vantagemElemental,
    critico,
    reducaoResistencia: Math.floor(reducao),
    danoFinal
  });
...
export function aplicarEfeitosStatus(habilidade, alvo, atacante) {
  ...
  const EFEITOS_STATUS = {
    ...
    aegis_sagrado: { nome: 'Aegis Sagrado', tipo: 'defensivo', escudo_percentual: 0.35, reflexo_dano: 0.15, icone: '🛡️✨' },
    corrente_temporal: { nome: 'Corrente Temporal', tipo: 'buff', bonus_agilidade: 0.20, reduz_cooldown: 1, icone: '🌊⏰' },
    transcendencia: { nome: 'Transcendência', tipo: 'buff', bonus_todos_stats: 0.50, icone: '🌟💫' },
    ...
    roubo_vida_intenso: { nome: 'Roubo de Vida Intenso', tipo: 'especial', percentual_roubo: 0.25, icone: '🩸🩸' },
  ...
  habilidade.efeitos_status.forEach(efeitoNome => {
    ...
    // Aplicar no alvo correto (self vs inimigo)
      ...
      battleLogger.logAplicarEfeito(atacante, efeitoInfo.nome, duracao, 'buff');
      ...
      battleLogger.logAplicarEfeito(alvo, efeitoInfo.nome, duracao, 'debuff');
...
export function usarHabilidade(atacante, habilidade, defensor, estado) {
  ...
  battleLogger.logUsarHabilidade(atacante, habilidade, custoEnergia);
  ...
  battleLogger.logEfeitosAtivos(atacante);
  battleLogger.logEfeitosAtivos(defensor);
...
export function ataqueBasico(atacante, defensor, estado) {
  ...
  const ataqueBasicoHabilidade = {
    ...
    multiplicador_stat: 2.5, // Força × 2.5
    stat_primario: 'forca',
    custo_energia: 0
  };
