...
export function calcularRecompensasTreino(poderOponente, dificuldade, vitoria) {
  ...
  const multiplicadores = {
    facil: { xp: 0.7, vinculo: 0.5, exaustao: 8 },
    normal: { xp: 1.0, vinculo: 1.0, exaustao: 12 },
    dificil: { xp: 1.5, vinculo: 1.5, exaustao: 18 }
  };
  ...
  if (vitoria) {
    ...
    const exaustao = mult.exaustao; // 8-18 de exaustão
    ...
    const exaustao = Math.floor(mult.exaustao * 1.5); // 50% mais exaustão
...
export function calcularPenalidadesAbandono(dificuldade) {
  const penalidades = {
    facil: { vinculo: -2, exaustao: 15 },
    normal: { vinculo: -3, exaustao: 20 },
    dificil: { vinculo: -5, exaustao: 25 }
  };
