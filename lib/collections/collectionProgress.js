...
function verificarColecaoElemento(criterio, avatares) {
  ...
  const avataresDoElemento = avatares.filter(av => {
    const valido = av.elemento === elemento && av.status !== 'vendendo' && !av.caido && av.vivo !== false;
    return valido;
  });

  console.log(`[COLECAO ELEMENTO] ${elemento}: ${avataresDoElemento.length}/${quantidade}`, {
    totalAvatares: avatares.length,
    avataresDoElemento: avataresDoElemento.map(av => ({ nome: av.nome, elemento: av.elemento, status: av.status, caido: av.caido, vivo: av.vivo }))
  });
...
function verificarColecaoRaridade(criterio, avatares) {
  ...
  const avataresDaRaridade = avatares.filter(av => {
    const valido = av.raridade === raridade && av.status !== 'vendendo' && !av.caido && av.vivo !== false;
    return valido;
  });

  console.log(`[COLECAO RARIDADE] ${raridade}: ${avataresDaRaridade.length}/${quantidade}`, {
    totalAvatares: avatares.length,
    avataresDaRaridade: avataresDaRaridade.map(av => ({ nome: av.nome, raridade: av.raridade, status: av.status }))
  });
...
function verificarColecaoCaracteristica(criterio, avatares) {
  ...
  switch (caracteristica) {
    case 'olhos':
      avataresComCaracteristica = avatares.filter(av =>
        av.olhos === valor && av.status !== 'vendendo' && !av.caido && av.vivo !== false
      );
      break;

    case 'temAsas':
      avataresComCaracteristica = avatares.filter(av =>
        av.temAsas === valor && av.status !== 'vendendo' && !av.caido && av.vivo !== false
      );
      break;

    case 'temChifres':
      avataresComCaracteristica = avatares.filter(av =>
        av.temChifres === valor && av.status !== 'vendendo' && !av.caido && av.vivo !== false
      );
      break;
  ...
  console.log(`[COLECAO CARACTERISTICA] ${caracteristica}=${valor}: ${avataresComCaracteristica.length}/${quantidade}`);
...
function verificarColecaoPoder(criterio, avatares) {
  ...
  const avataresComPoder = avatares.filter(av => {
    if (av.status === 'vendendo' || av.caido || av.vivo === false) return false;
    const poder = calcularPoderTotal(av);
    return poder >= poderMinimo;
  });

  console.log(`[COLECAO PODER] ${poderMinimo}+: ${avataresComPoder.length}/${quantidade}`, {
    avataresComPoder: avataresComPoder.map(av => ({ nome: av.nome, poder: calcularPoderTotal(av) }))
  });
...
function verificarColecaoEspecial(criterio, avatares) {
  ...
  console.log(`[COLECAO ESPECIAL] Avatares válidos: ${avataresValidos.length}/${avatares.length}`);
