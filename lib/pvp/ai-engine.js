 ...
 */
export function escolherAcaoIA(avatarIA, avatarOponente, personalidade) {
  ...
  // 1. HP CRÍTICO - priorizar defesa/cura
  if (hpPercent < config.limiar_hp_critico) {
    ...
    if (avatarIA.habilidades && avatarIA.habilidades.length > 0) {
      for (let i = 0; i < avatarIA.habilidades.length; i++) {
        ...
        if (hab.tipo === 'Suporte' && energia >= custo) {
          return { acao: 'ability', habilidadeIndex: i };
        }
      }
    }

    // Se não tem cura, 70% chance de defender
    if (Math.random() < 0.7) {
      return { acao: 'defend' };
    }
  }
  ...
  if (hpOponentePercent < 25 && hpPercent > 30) {
    ...
    if (avatarIA.habilidades && avatarIA.habilidades.length > 0) {
      ...
      for (let i = 0; i < avatarIA.habilidades.length; i++) {
        ...
        if (hab.tipo === 'Ofensiva' && energia >= custo && dano > maiorDano) {
          melhorHab = i;
          maiorDano = dano;
        }
      }
  ...
  // 3. ENERGIA BAIXA - recuperar
  if (energia < 20) {
    return { acao: 'defend' };
  }

  // 4. DECISÃO BASEADA EM PERSONALIDADE
  const rand = Math.random();
  let acumulado = 0;
  ...
  if (rand < (acumulado += config.prioridades.habilidade)) {
    if (avatarIA.habilidades && avatarIA.habilidades.length > 0) {
      // Escolher habilidade disponível aleatória
    ...
    // Sem habilidades disponíveis, atacar
    return { acao: 'attack' };
  }
