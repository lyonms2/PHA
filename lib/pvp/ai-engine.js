...
import { calcularDano } from '../arena/batalhaEngine';

/**
 * Simula dano de uma habilidade sem executá-la
 * Usado pela IA para avaliar qual habilidade usar
 */
function simularDano(habilidade, atacante, defensor) {
  // Reutiliza função completa de cálculo de dano
  // Sempre sem crítico (false) para estimativa conservadora
 ...
 */
function escolherMelhorHabilidade(habilidades, situacao, personalidade, avatarIA, avatarOponente) {
  ...
  habilidades.forEach(hab => {
    ...
    // Bonus por dano alto - SIMULAR DANO REAL
    if (hab.tipo === 'Ofensiva') {
      const danoEstimado = simularDano(hab, avatarIA, avatarOponente);
      pontuacao += danoEstimado / 10; // Dano é muito importante
    }
...
export function decidirProximaAcao(estadoBatalha, avatarIA, avatarOponente, personalidade, cooldowns = {}) {
  ...
  if (situacao.hpOponentePercent < 25 && hpPercent > 30) {
    const habilidadesOfensivas = avatarIA.habilidades.filter(h => h.tipo === 'Ofensiva');
    ...
    if (ofensivasDisponiveis.length > 0) {
      // Escolher a mais forte - SIMULAR DANO REAL
      const maisForte = ofensivasDisponiveis.reduce((prev, current) => {
        const danoPrev = simularDano(prev, avatarIA, avatarOponente);
        const danoCurrent = simularDano(current, avatarIA, avatarOponente);
        return danoCurrent > danoPrev ? current : prev;
      });
      return {
        tipo: 'habilidade',
  ...
  if (rand < (acumulado += prioridades.habilidade)) {
    if (habilidadesDisponiveis.length > 0) {
      const habilidade = escolherMelhorHabilidade(habilidadesDisponiveis, situacao, personalidade, avatarIA, avatarOponente);
      return {
        tipo: 'habilidade',
        habilidade,
...
export function escolherAcaoIA(avatarIA, avatarOponente, personalidade, cooldowns = {}) {
  ...
  const energia = avatarIA.energy !== undefined ? avatarIA.energy : 100;
  ...
  if (hpOponentePercent < 25 && hpPercent > 30) {
    // Procurar habilidade ofensiva mais forte - SIMULAR DANO REAL
    if (avatarIA.habilidades && avatarIA.habilidades.length > 0) {
      let melhorHab = -1;
      let maiorDano = 0;
      ...
      for (let i = 0; i < avatarIA.habilidades.length; i++) {
        ...
        if (hab.tipo === 'Ofensiva' && energia >= custo && !emCooldown) {
          const danoEstimado = simularDano(hab, avatarIA, avatarOponente);
          if (danoEstimado > maiorDano) {
            melhorHab = i;
            maiorDano = danoEstimado;
          }
        }
      }
