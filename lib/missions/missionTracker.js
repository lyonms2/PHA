...
export async function trackMissionProgress(userId, tipoEvento, incremento = 1) {
  try {
    ...
    // Tratamento especial para erros de conectividade (offline)
    if (error.code === 'unavailable' || error.message?.includes('offline')) {
      console.warn('⚠️ [MISSÃO] Cliente offline - rastreamento de missão ignorado (não afeta gameplay)');
      return { success: false, reason: 'offline', silenced: true };
    }

    // Outros erros logam normalmente
